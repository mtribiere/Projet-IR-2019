...
void computeStrategy(Dog *dogInfos, Entity *entityAround, int numberOfEntity)
{
  ...
  if(dogInfos->dogType == 2 || dogInfos->dogType == 3) {
    ...
    if(dogInfos->state == 0){
      ...
      //Aller au meetPoint
      dogInfos->targetPositionX = MAP_SIZE_X/30;
      dogInfos->targetPositionY = MAP_SIZE_Y/2;
    ...
    if(dogInfos->state == 4){
        ...
        if((tmpIdSheep > 0) && isInBase(entityAround[tmpIdSheep].positionX,entityAround[tmpIdSheep].positionY)){
          ...
          if(!(isInBase(entityAround[tmpIdSheep].positionX,entityAround[tmpIdSheep].positionY))) dogInfos->state = 5;
      }

    }
  /*  int posX = dogInfos->targetPositionX;
    int posY = dogInfos->targetPositionY;  */
    int posX = entityAround[dogInfos->targetedSheepId].positionX;
    int posY = entityAround[dogInfos->targetedSheepId].positionY;
    int posDX = (dogInfos->entity).positionX;
    int posDY = (dogInfos->entity).positionY;
    int posTDX = dogInfos->targetPositionX;
    int posTDY = dogInfos->targetPositionY;

    printf("pos X = %d ; pos Y = %d\n",posX,posY);
    printf("pos dog X = %d ; pos dog Y = %d\n",posDX,posDY);
    printf("pos target dog X = %d ; pos target dog Y = %d\n",posTDX,posTDY);

    if(!(isInBase(posX,posY)) || ((isPushingPositionReached(posDX,posDY,posTDX,posTDY) == 1) && (dogInfos->state == 4)) || ((findIdOfSheep(entityAround,numberOfEntity,dogInfos->targetedSheepId) == -1) && /*((dogInfos->state == 4) || */(dogInfos->state == 5))){
      // Brebis en position
